...
describe('useInitGameState', () => {
  const { GenerateGameWordCode, GenerateRandomGameWordCode, GetGameWordCodeValidationRule, GetGameWordValidationRule } = useInitGameState;

  it.each`
    providedGameWord
    ${'AREOLA'}
    ${'FARTING'}
  `('WHEN providedGameWord=$providedGameWord, THEN is considered valid and produces valid code', ({ providedGameWord }) => {
